...
log_format = (
    "[%(asctime)s] %(levelname)s %(source)s %(funcName)s:%(lineno)d - %(message)s"
)
...
class CustomLogger:
    ...
    def __init__(
        ...
        base_dir: str = "./logs",
        log_rotate_days: int = Settings.LOG_ROTATE_DAYS,
    ):
        """
    ...
    def _setup_file_handler(self, log_rotate_days: int):
        ...
        log_file_path = os.path.join(
            self.log_directory,
            f"{self.source}_{datetime.now().strftime('%Y_%m_%d')}.log",
        )
